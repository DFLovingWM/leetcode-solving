...
var characterReplacement = function(s, K) {
  ...
  while (right < s.length) {
    ...
    ++right
    if (right - left - maxCount <= K) { // 能替换所有非最多字符
      res = Math.max(res, right - left)
    }
    
    while (right - left - maxCount > K && left <= right) { // 不能，则减少头字符直到删除了一个非最多字符
      const oldChar = s[left++]
      count.set(oldChar, count.get(oldChar) - 1)

      // 重新找出最多字符（下面这句代码可以不需要，能剩下很多时间）
...
[
  ...
  console.log(characterReplacement(...input))
})