...
var maxAverageRatio = function(classes, extraStudents) {
  const queue = new PriorityQueue((A, B) => A[2] > B[2]); // 增幅更大
  for (const [a, b] of classes) {
    queue.add(getState(a, b));
  }
  // 逐个学霸加
  while (extraStudents--) {
    const [a, b] = queue.poll();
    queue.add(getState(a + 1, b + 1));
  }
  // 最后求结果
  let res = 0;
  while (!queue.empty()) {
