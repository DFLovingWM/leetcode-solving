...
var minPushBox = function (grid) {
  ...
  const visit = Array.from({ length: R }, () => Array.from({ length: C }, () => // 是否已遍历（状态用四维表示）
  ...
  const queue = new PriorityQueue((a, b) => a[4] < b[4])
  queue.add([x, y, bx, by, 0])
  ...
  while (queue.size() > 0) {
    const [currX, currY, currBoxX, currBoxY, step] = queue.poll()

    for (const dir of DIRS) {
      const [nx, ny] = [currX + dir[0], currY + dir[1]]
      const [nbx, nby] = [nx + dir[0], ny + dir[1]]
      if (!canWalk(nx, ny)) continue

      if (nx === currBoxX && ny === currBoxY) { // 推箱子
        if (!canWalk(nbx, nby) || visit[nx][ny][nbx][nby]) continue
        if (grid[nbx][nby] === 'T') { // 如果推到终点了
          return step + 1
        }
        queue.add([nx, ny, nbx, nby, step + 1])
        visit[nx][ny][nbx][nby] = true
      } else { // 单纯走路
        if (visit[nx][ny][currBoxX][currBoxY]) continue
        queue.add([nx, ny, currBoxX, currBoxY, step])
        visit[nx][ny][currBoxX][currBoxY] = true
      }
    }
  }
